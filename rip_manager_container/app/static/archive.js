...
function renderList(){q("#archiveCount").textContent=`${media.length} disc${media.length===1?"":"s"} · current and completed`;list.innerHTML=media.map((x,i)=>`<button class="archive-card" data-media="${i}"><span><span class="archive-card-title">${E(title(x))}</span><span class="archive-card-meta">${E(x.media_type||"disc")} · ${E(x.state||"")} · ${E(x.node_id||"")}</span></span><span class="archive-statuses"><span class="archive-status"><small>Front</small><b class="${x.front_url?"archive-ok":"archive-missing"}">${x.front_url?"✓":"×"}</b></span><span class="archive-status"><small>Rear</small><b class="${x.rear_url?"archive-ok":"archive-missing"}">${x.rear_url?"✓":"×"}</b></span><span class="archive-status"><small>Extras</small><b>${x.extra_urls.length}</b></span></span></button>`).join("");}
...
function renderDetail(){const x=current,front=q("#archiveFront"),rear=q("#archiveRear");q("#archiveDetailName").textContent=title(x);q("#archiveDetailMeta").textContent=`${x.media_type||"disc"} · ${x.state||""} · UPC ${x.barcode||"not recorded"}`;front.className=`archive-slot${x.front_url?" has":""}`;front.innerHTML=x.front_url?image(x.front_url,"front"):empty("front");front.toggleAttribute("data-add",!x.front_url);if(!x.front_url)front.dataset.add="front";rear.className=`archive-slot${x.rear_url?" has":""}`;rear.innerHTML=x.rear_url?image(x.rear_url,"rear"):empty("rear");rear.toggleAttribute("data-add",!x.rear_url);if(!x.rear_url)rear.dataset.add="rear";q("#archiveExtras").innerHTML=`<button class="archive-slot" data-add="extra">${empty("extra")}</button>`+x.extra_urls.map((src,i)=>`<span class="archive-slot has">${image(src,"extra",i)}</span>`).join("")}
