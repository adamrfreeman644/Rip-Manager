...
DRIVE_MAP_FILE = Path(os.getenv("RIP_NODE_DRIVE_MAP", "/opt/rip-node/drive-map.json"))
DEFAULT_DRIVE_MAP = {
...
def load_drive_map() -> dict:
    try:
        if DRIVE_MAP_FILE.exists():
            payload = json.loads(DRIVE_MAP_FILE.read_text(encoding="utf-8"))
...
RIP_ROOT = Path(os.getenv("RIP_NODE_OUTPUT", "/mnt/ripping"))
...
def scan_optical_devices() -> list[dict]:
    ...
    found = []
    for path in sorted(Path("/dev").glob("sr*"), key=lambda p: p.name):
        device = str(path)
...
def drive_mapping(authorization: Optional[str] = Header(default=None)):
    require_token(authorization)
...
def set_drive_mapping(
    ...
    return {"ok": True, "message": f"{device} assigned to {name}", **drive_mapping(authorization)}
...
def remove_drive_mapping(drive_name: str, authorization: Optional[str] = Header(default=None)):
    ...
    with drive_map_lock:
        ...
        DRIVE_MAP.pop(name)
        save_drive_map()
    return {"ok": True, "message": f"{name} removed", **drive_mapping(authorization)}
