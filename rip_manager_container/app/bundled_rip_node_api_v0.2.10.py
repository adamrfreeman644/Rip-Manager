...
What changed in 0.2.10
----------------------
- Accepts readable short MKV extras instead of failing an otherwise healthy rip
  solely because one title is smaller than 5 MiB.
- Still requires at least one readable, normal-sized MKV, so a genuinely empty
  or truncated rip cannot be marked complete.

What changed in 0.2.9
---------------------
- Verifies suspicious open/empty tray reports from USB optical bridges against
  the block device. A physically closed drive containing readable media is no
  longer stuck as "tray open" and prepared rips can auto-start normally.
- Rip startup uses the verified media result instead of rejecting a disc from
  the raw, unreliable tray ioctl alone.
...
# Verification: every produced MKV must exist and contain readable video. At
# least one must also be non-trivially sized. Optical discs commonly include
# very small but valid logo/menu/bonus titles, so those are warnings rather
# than grounds for rejecting an otherwise healthy rip.
...
def media_has_capacity(raw_device: str) -> bool:
    """Return True only when the kernel exposes readable media capacity.

    Several USB optical bridges used by trayless/manual-close drives return
    CDS_TRAY_OPEN even while a disc is physically loaded.  A successful,
    non-zero block capacity is stronger evidence than that unreliable ioctl.
    An actually open or empty tray returns no capacity, so normal tray controls
    retain their existing behaviour.
    """
    try:
        result = run_command(
            ["blockdev", "--getsize64", raw_device],
            timeout=min(MEDIA_PROBE_TIMEOUT, 5),
        )
...
def read_disc_status(device: str) -> dict:
    ...
    # Some USB bridges claim the tray is open (or empty) even though their
    # manual-close mechanism is shut and a disc is readable. Verify those
    # negative states before publishing them to Manager.
    if tray in {"open", "empty"} and media_has_capacity(raw_device):
        verified = read_disc_label(raw_device, device)
        return {
            **verified,
            "tray": "disc",
            "reported_tray": tray,
            "tray_status_reliable": False,
            "status_message": verified.get("status_message")
            or "Disc detected; the USB bridge reported an incorrect tray state",
        }
...
def verify_rip(output_dir: Path) -> dict:
    ...
    - at least one readable MKV must be larger than the configured minimum

    A readable title below the size threshold is retained as a legitimate
    short extra and reported as a warning. This avoids repeatedly rejecting a
    complete feature because the disc also contains a tiny logo or menu title.
    ...
    for mkv in mkvs:
        ...
        if size < MIN_VERIFIED_MKV_BYTES:
            item["small_valid_title"] = True
            result["small_files"] += 1
            result["warnings"].append(
                f"Accepted readable short title {mkv.name} ({size} bytes)"
            )
        else:
            item["small_valid_title"] = False

    substantial_files = len(mkvs) - result["small_files"]
    if substantial_files < 1:
        result["error"] = (
            "No substantial MKV title was produced; all readable files were "
            f"smaller than {MIN_VERIFIED_MKV_BYTES} bytes"
        )
        return result
    ...
    try:
        ...
        if job.state == "cancelling":
            ...
            if job.verification.get("ok"):
                ...
                short_titles = int(job.verification.get("small_files") or 0)
                suffix = (
                    f"; accepted {short_titles} readable short title"
                    f"{'s' if short_titles != 1 else ''}"
                    if short_titles else ""
                )
                job.last_message = (
                    f"{job.request.media_type or 'movie'} rip completed and output verified{suffix}"
                )
...
def start_rip(
    ...
    media_status = read_disc_status(device)
    tray = media_status.get("tray", "unknown")
    media_present = bool(media_status.get("present"))
    if tray == "open" and not media_present:
        raise HTTPException(status_code=409, detail=f"{name} has its tray open")
    if tray == "empty" and not media_present:
